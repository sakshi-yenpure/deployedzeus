...
import { yfinanceService, StockData, NewsItem, MutualFundData } from '../services/yfinanceService';
...
const Home: React.FC = () => {
    ...
    const loadData = async () => {
      try {
        ...
        if (mutualFunds.length === 0) {
          const mfData = await yfinanceService.getMutualFunds();
          setMutualFunds(mfData.slice(0, 6));
        }
      } catch (err) {
  ...
  }, [trending.length, news.length, mutualFunds.length]);
  ...
  return (
    <HomeContainer>
        ...
        <ChartCard>
          <div style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 style={{ fontSize: '1.2rem', fontWeight: 900, letterSpacing: '2px', textTransform: 'uppercase' }}>
              Latest <span>Financial News</span>
            </h2>
            <Link to="/news" style={{ color: '#00f2fe', textDecoration: 'none', fontSize: '0.8rem', fontWeight: 'bold' }}>View All →</Link>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem', maxHeight: '380px', overflowY: 'auto', paddingRight: '1rem', scrollbarWidth: 'thin' }}>
            {news.length > 0 ? news.map((item, idx) => (
              <motion.div
                ...
                whileHover={{ background: 'rgba(255,255,255,0.05)', borderColor: 'rgba(0, 242, 254, 0.2)', x: 5 }}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  <span style={{ fontSize: '0.7rem', color: '#00f2fe', fontWeight: 'bold', textTransform: 'uppercase' }}>{item.category}</span>
                  <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>{item.date}</span>
                </div>
                <h3 style={{ fontSize: '1.1rem', fontWeight: 'bold', marginBottom: '0.5rem', lineHeight: 1.4 }}>{item.title}</h3>
                <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase' }}>{item.source}</div>
              </motion.div>
            )) : <div style={{ color: 'rgba(255,255,255,0.5)', textAlign: 'center', padding: '2rem' }}>Loading News Stream...</div>}
          </div>
        </ChartCard>
      ...
      <GridContainer>
        <BaseCard style={{ gridColumn: 'span 12', overflowX: 'auto', padding: '1rem' }} whileHover={{ borderColor: 'rgba(0, 242, 254, 0.2)' }}>
          <table style={{ width: '100%', minWidth: '800px', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.5)', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '1px' }}>
                <th style={{ padding: '1rem', fontWeight: 600 }}>Indices</th>
                <th style={{ padding: '1rem', fontWeight: 600 }}>Price</th>
                <th style={{ padding: '1rem', fontWeight: 600 }}>1D Change %</th>
                <th style={{ padding: '1rem', fontWeight: 600 }}>1W Change %</th>
                <th style={{ padding: '1rem', fontWeight: 600 }}>1Y Change %</th>
                <th style={{ padding: '1rem', fontWeight: 600 }}>3Y Change %</th>
              </tr>
            </thead>
            <tbody>
              {[
                { name: 'Nifty 50', price: '23,306.45', c1d: '+1.72', c1w: '-2.84', c1y: '-3.15', c3y: '+35.22' },
                { name: 'Nifty Midcap 150', price: '20,397.75', c1d: '+2.34', c1w: '-2.14', c1y: '+2.64', c3y: '+78.52' },
                { name: 'Nifty Smallcap 250', price: '14,937.20', c1d: '+2.49', c1w: '-2.12', c1y: '-5.32', c3y: '+67.37' },
                { name: 'Nifty Next 50', price: '63,620.60', c1d: '+2.02', c1w: '-4.09', c1y: '-2.01', c3y: '+66.17' },
                { name: 'Nifty 500', price: '21,478.20', c1d: '+1.95', c1w: '-2.78', c1y: '-2.19', c3y: '+47.54' },
                { name: 'Nifty 100', price: '23,915.15', c1d: '+1.77', c1w: '-3.05', c1y: '-2.95', c3y: '+39.61' },
                { name: 'Nifty 200', price: '13,072.60', c1d: '+1.87', c1w: '-2.85', c1y: '-1.95', c3y: '+45.36' },
                { name: 'Nifty LargeMidcap 250', price: '15,268.35', c1d: '+2.06', c1w: '-2.59', c1y: '-0.14', c3y: '+58.48' },
              ].map((idx, i) => (
                <tr key={idx.name} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  <td style={{ padding: '1rem', fontWeight: 600, color: '#fff' }}>{idx.name}</td>
                  <td style={{ padding: '1rem', fontFamily: 'JetBrains Mono', fontWeight: 900 }}>{idx.price}</td>
                  <td style={{ padding: '1rem', color: idx.c1d.startsWith('+') ? '#00ffa3' : '#ff2e63', fontWeight: 900 }}>{idx.c1d}</td>
                  <td style={{ padding: '1rem', color: idx.c1w.startsWith('+') ? '#00ffa3' : '#ff2e63', fontWeight: 900 }}>{idx.c1w}</td>
                  <td style={{ padding: '1rem', color: idx.c1y.startsWith('+') ? '#00ffa3' : '#ff2e63', fontWeight: 900 }}>{idx.c1y}</td>
                  <td style={{ padding: '1rem', color: idx.c3y.startsWith('+') ? '#00ffa3' : '#ff2e63', fontWeight: 900 }}>{idx.c3y}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <GridContainer>
        {mutualFunds.map((fund, idx) => (
          <BaseCard 
            ...
            whileHover={{ y: -10, borderColor: 'rgba(112,40,228,0.3)' }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ fontWeight: 900, fontSize: '1.2rem', color: '#fff' }}>{fund.symbol}</div>
                <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', marginTop: '0.2rem' }}>{fund.category}</div>
              </div>
              <div style={{ display: 'flex', gap: '2px' }}>
                {[...Array(5)].map((_, i) => (
                  <span key={i} style={{ color: i < fund.rating ? '#ffc107' : 'rgba(255,255,255,0.1)', fontSize: '1rem' }}>★</span>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '1rem' }}>
              <div>
                <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', marginBottom: '0.2rem' }}>NAV</div>
                <div style={{ fontSize: '1.5rem', fontWeight: 900, fontFamily: 'JetBrains Mono' }}>
                  {yfinanceService.formatStockPrice(fund.currentPrice, fund.symbol)}
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '0.9rem', fontWeight: 800, color: fund.change >= 0 ? '#00ffa3' : '#ff2e63' }}>
